      ...
      <div className="overflow-x-auto">
        <div className="space-y-3">
          {filteredSales.map((sale) => (
            <div key={sale.id} className="p-4 border border-gray-200 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors flex justify-between items-center">
              <div className="flex-1">
                <div className="text-lg font-medium text-gray-900 dark:text-slate-100">
                  {sale.dayName}: {sale.productName}, {(sale.unitPrice || sale.price || 0).toFixed(2)} MAD, {sale.totalPrice.toFixed(2)} MAD
                </div>
                <div className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                  {formatDate(sale.date)} • Catégorie: {sale.category} • Quantité: {sale.quantity || 1}
                </div>
              </div>
              <button
                onClick={() => onDeleteSale(sale.id)}
                className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 font-medium text-sm ml-4 transition-colors"
